...
class MapperProcessor(private val acc: Accumulator) : JavaVisitor<ExecutionContext>() {

    /**
     * Modify the mapper declaration file with a modified mix of methods and fields from itself
     * plus the mapper implementation generated by mapstruct.
     */
    override fun visitCompilationUnit(
        ...
        // declaration file. Delete the now-redundant generated source by returning null.
        ...
        if (spyFieldNames.isNotEmpty()) {
            log.fine(
                "[PurgeMapstruct] Found spy fields in ${mapperDeclFile_.sourcePath}: $spyFieldNames"
            )
        }
        ...
        try {
            ...
            var clazz =
                mapperImplClass
                    ...
                    .withLeadingAnnotations(
                        (mapperDeclClass.leadingAnnotations.filter {
                                excludeMapstructAnnotations(it)
                            } +
            ...
            val qualified =
                object : JavaVisitor<ExecutionContext>() {
                    override fun visitIdentifier(
                        ...
                        val fqType =
                            identifier.type as? JavaType.FullyQualified ?: return identifier
                        val fqn = fqType.fullyQualifiedName
                        if (!fqn.contains(".")) return identifier
                        val nearestFa =
                            cursor.firstEnclosing(J.FieldAccess::class.java)
            ...
            log.severe(
                "Error processing @Mapper class ${visited.classes[0].name}: ${e.message}"
            )
    ...
    /**
     * Replaces references like `UserMapperImpl.class` to `UserMapper.class`, for both simple-name
     * and fully-qualified targets.
     */
    override fun visitFieldAccess(fieldAccess_: J.FieldAccess, ctx: ExecutionContext): J {
        ...
        if (target is J.FieldAccess) {
            ...
            val superSimpleName = extractSimpleName(superFqn)
            val superType = JavaType.buildType(superFqn)
            val newTarget = target.withName(target.name.withSimpleName(superSimpleName)).withType(superType)
    ...
    override fun visitImport(imp: J.Import, ctx: ExecutionContext): J {
     ...
     */
    override fun visitInstanceOf(instanceOf_: J.InstanceOf, ctx: ExecutionContext): J {
    ...
    private fun extractFqnFromFieldAccess(fieldAccess: J.FieldAccess): String? {
        ...
        while (true) {
            ...
            when (val target = current.target) {
                is J.FieldAccess -> current = target
                is J.Identifier -> {
    ...
    private fun replaceImportQualid(import_: J.Import, superFqn: String): J.Import {
        val superSimpleName = extractSimpleName(superFqn)
     ...
     * declared `@Spy` field, rebuilds the tree as `doReturn(value).when(spy).method(args)` and
     * adds the necessary Mockito static import. Returns `null` when the pattern doesn't match.
     */
    private fun rewriteWhenOnSpyIfApplicable(
        ...
        val spyMethodCall =
            whenInvocation.arguments[0] as? J.MethodInvocation ?: return null
        ...
        val mockitoType =
            JavaType.buildType("org.mockito.Mockito") as JavaType.FullyQualified
        val stubberType =
        ...
        val newWhenCall =
            J.MethodInvocation(
                ...
                JContainer.build(
                    Space.EMPTY,
                    listOf(JRightPadded.build(spyIdent.withPrefix(Space.EMPTY))),
                    Markers.EMPTY,
                ),
    ...
    companion object {
        ...
        private fun transformMapperDeclMethod(
            mapperDeclMethod: J.MethodDeclaration
        ): J.MethodDeclaration? {
            ...
            method =
                method.withLeadingAnnotations(
                    ListUtils.filter(method.leadingAnnotations) {
                        excludeMapstructAnnotations(it)
                    }
                )
                ...
                ListUtils.map(method.parameters) { param ->
                    if (param is J.VariableDeclarations) {
                        param.withLeadingAnnotations(
                            ListUtils.filter(param.leadingAnnotations) {
                                excludeMapstructAnnotations(it)
                            }
                        )
                    } else {
        ...
        private fun transformMapperDeclInterfaceField(
            mapperDeclField: J.VariableDeclarations
            ...
            val modifiersSetManually =
                accessModifiers +
                    setOf(J.Modifier.Type.Static, J.Modifier.Type.Final)

            val modifiers = mutableListOf<J.Modifier>()

            val existingAccess =
                mapperDeclField.modifiers.firstOrNull { it.type in accessModifiers }
            modifiers.add(
                existingAccess
                    ?: J.Modifier(
                        UUID.randomUUID(),
        ...
        private fun transformMapperImplMethod(
            ...
            val filteredAnnotations =
                ListUtils.map(originalAnnotations) { ann ->
                    if (ann.simpleName == "Override" ||
                        TypeUtils.isOfClassType(ann.type, "java.lang.Override")
                    )
                        null
                    else ann
                ...
                for (stmt in clazz.body.statements) {
                    ...
                    val isSpy =
                        stmt.leadingAnnotations.any { a ->
                            a.simpleName == "Spy" ||
                                TypeUtils.isOfClassType(a.type, "org.mockito.Spy")
                        }
                    if (!isSpy) continue
        ...
        private fun isMappersGetMapper(invocation: J.MethodInvocation): Boolean {
            ...
            if (methodType?.declaringType != null) {
                return methodType.declaringType.fullyQualifiedName ==
         ...
         */
        private fun buildFqnTypeTree(fqn: String, prefix: Space): TypeTree {
            val parts = fqn.split(".")
