...
public class MapperProcessor extends JavaVisitor<ExecutionContext> {
    ...
    public J visitMethodInvocation(J.MethodInvocation method, ExecutionContext ctx) {
        ...
        // practice for spies.
        J.MethodInvocation rewrittenSpyStub = rewriteWhenOnSpyIfApplicable(invocation, ctx);
    ...
    public J visitInstanceOf(J.InstanceOf instanceOf_, ExecutionContext ctx) {
        J visited = super.visitInstanceOf(instanceOf_, ctx);
        if (!(visited instanceof J.InstanceOf instanceOf)) {
            return visited;
        }

        // Replace the type in instanceof checks
        J clazzExpr = instanceOf.getClazz();
    ...
    private static Set<String> collectSpyFieldNames(J.CompilationUnit cu) {
        ...
        for (J.ClassDeclaration clazz : cu.getClasses()) {
            for (Statement stmt : clazz.getBody().getStatements()) {
                ...
                for (J.VariableDeclarations.NamedVariable nv : varDecl.getVariables()) {
    ...
    private J.@Nullable MethodInvocation rewriteWhenOnSpyIfApplicable(J.MethodInvocation invocation, ExecutionContext ctx) {
        ...
        Expression spyRef = spyMethodCall.getSelect();
        if (!(spyRef instanceof J.Identifier spyIdent)) {
            return null;
        }
        ...
        String doStubName = "thenReturn".equals(stubName) ? "doReturn" : "doThrow";
        ...
        for (int i = 0; i < spyArgs.size(); i++) {
            Space prefix = i == 0 ? Space.EMPTY : Space.SINGLE_SPACE;
            paddedFinalArgs.add(JRightPadded.build(spyArgs.get(i).withPrefix(prefix)));
        }
