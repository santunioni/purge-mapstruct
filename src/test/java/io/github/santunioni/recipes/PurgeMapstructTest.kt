...
internal class PurgeMapstructTest : RewriteTest {
    override fun defaults(spec: RecipeSpec) {
        spec
            ...
            .parser(
                JavaParser.fromJavaVersion().classpath("mapstruct", "lombok", "junit-jupiter-api")
            )
    }
    ...
    fun shouldReplaceInterfaceMapper() {
        val makeAvailableUserDto: SourceSpecs =
            java(readResource("fixtures/shouldReplaceInterfaceMapper/context/UserDto.java")) {
                spec ->
                spec.path("src/main/java/com/santunioni/fixtures/UserDto.java")
            }

        val makeAvailableUserEntity: SourceSpecs =
            java(readResource("fixtures/shouldReplaceInterfaceMapper/context/UserEntity.java")) {
                spec ->
    ...
    fun shouldReplaceMappersGetMapper() {
        ...
        val makeAvailableGeneratedClass =
            java(
                readResource(
                    "fixtures/shouldReplaceMappersGetMapper/context/CustomerMapperImpl.java"
                ),
                null as String?,
            ) { spec ->
                spec.path(
    ...
    fun shouldRemoveAfterMappingDecorators() {
        ...
        val makeAvailableCustomerEntity: SourceSpecs =
            java(
                readResource(
                    "fixtures/shouldRemoveAfterMappingDecorators/context/CustomerEntity.java"
                )
            ) { spec ->
                spec.path("src/main/java/com/santunioni/fixtures/CustomerEntity.java")
            }
    ...
    fun shouldRewriteWhenOnSpyToDoReturn() {
        ...
        val makeAvailableGeneratedClass =
            java(
                readResource(
                    "fixtures/shouldRewriteWhenOnSpyToDoReturn/context/UserMapperImpl.java"
                ),
                null as String?,
            ) { spec ->
                spec.path(
        ...
        rewriteRun(
            ...
            java(
                readResource(
                    "fixtures/shouldRewriteWhenOnSpyToDoReturn/before/UserMapperSpyTest.java"
                ),
                readResource(
                    "fixtures/shouldRewriteWhenOnSpyToDoReturn/after/UserMapperSpyTest.java"
                ),
            ) { spec ->
    ...
    companion object {
        private fun readResource(resource: String): String =
            PurgeMapstructTest::class.java.classLoader
                .getResourceAsStream(resource)!!
                .use { stream -> String(stream.readAllBytes(), StandardCharsets.UTF_8) }
    }
}
